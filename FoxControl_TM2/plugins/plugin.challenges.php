...
class plugin_challenges extends FoxControlPlugin {
	...
	public function onChallengeListModified($args) {
	...
	public function writeChallenges() {
		...
		for($i = 0; $i < count($challenge_list); $i++) {
			//Select Votes
			$mysql = mysqli_query($this->db, "SELECT * FROM `karma` WHERE challengeid = '".$challenge_list[$i]['UId']."' AND playerlogin = 'karma_total'");
	...
	public function searchWriteChallenges($search) {
		...
		foreach($this->challenges as $key => $value) {
			if(strpos(strtolower(' '.$this->challenges[$key]['Name'].$this->challenges[$key]['Author'].$this->challenges[$key]['FileName'].$this->challenges[$key]['Environnement']), strtolower($search), 0)) {
				//WRITE SEARCH ARRAY
				$this->searchWriteChallenges[] = array('Name' => $this->challenges[$key]['Name'], 'FileName' => $this->challenges[$key]['FileName'], 'Author' => $this->challenges[$key]['Author'], 'Environnement' => $this->challenges[$key]['Environnement'], 'Karma' => $this->challenges[$key]['Karma']);
				$this->challengesFound = true;
			}
		}
		
		if($this->challengesFound == false) {
			$this->searchWriteChallenges[] = array('Name' => 'No Maps found!', 'FileName' => '', 'Author' => '', 'Environnement' => '', 'Karma' => '');
		}
	}
	...
	public function displayList($login) {
		...
		if(isset($this->searchWriteChallenges[0])) {
			$window->title('$fffMaps Search');
		}else {
			$window->title('$fffMaps');
		}
		...
		if($this->challengesFound == false AND isset($this->searchWriteChallenges[0])) {
			...
			$window->content('<td width="3">$iID</td><td width="25">$iMapname</td><td width="1"/><td width="11">$iAuthor</td><td width="1"/><td width="9">$iEnvironment</td><td width="1"></td><td width="10">$iVotes</td><td width="10">$iActions</td>');
		}else {
		...
		for($i = 0; isset($challengesArray[$curr_challid]) && $i <= 24; $i++)
		{
			...
			if($this->challengesFound == false AND isset($this->searchWriteChallenges[0])) {
				...
				if($admin == true) {
					if($this->instance()->pluginIsActive('plugin.jukebox.php') == true) {
						$window->content('<td width="3">'.($curr_challid + 1).'</td><td width="25" id="'.$chall_ml_id.'">'.htmlspecialchars($challengesArray[$curr_challid]['Name']).'</td><td width="1"/><td width="11">'.htmlspecialchars($challengesArray[$curr_challid]['Author']).'</td><td width="1"/><td width="9">'.$challengesArray[$curr_challid]['Environnement'].'</td><td width="1"></td><td width="10">'.$challengesArray[$curr_challid]['Karma'].'</td><td width="7" id="'.$delete_id.'" align="center">Delete</td>');
					}else {
				...
				//DISPLAY LIST FOR PLAYER
					...
					if($this->instance()->pluginIsActive('plugin.jukebox.php') == true) {
						$window->content('<td width="3">'.($curr_challid + 1).'</td><td width="30" id="'.$chall_ml_id.'">'.htmlspecialchars($challengesArray[$curr_challid]['Name']).'</td><td width="1"/><td width="13">'.htmlspecialchars($challengesArray[$curr_challid]['Author']).'</td><td width="1"/><td width="10">'.$challengesArray[$curr_challid]['Environnement'].'</td><td width="1"></td><td width="12">'.$challengesArray[$curr_challid]['Karma'].'</td>');
					}else {
						$window->content('<td width="3">'.($curr_challid + 1).'</td><td width="30">'.htmlspecialchars($challengesArray[$curr_challid]['Name']).'</td><td width="1"/><td width="13">'.htmlspecialchars($challengesArray[$curr_challid]['Author']).'</td><td width="1"/><td width="10">'.$challengesArray[$curr_challid]['Environnement'].'</td><td width="1"></td><td width="12">'.$challengesArray[$curr_challid]['Karma'].'</td>');
					}
				}
			}
